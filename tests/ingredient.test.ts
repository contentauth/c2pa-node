...
const thumbnailJumbfMatcher =
  /^self#jumbf=\/c2pa\/contentauth(:urn:uuid)?:\w{8}\-\w{4}-\w{4}-\w{4}-\w{12}\/c2pa\.assertions\/c2pa\.thumbnail\.claim\.jpeg$/;
const MANIFEST_DATA_IDENTIFIER = 'manifest_data.c2pa';
...
describe('createIngredient()', () => {
  ...
  describe('from buffer', () => {
    test('should read a JPEG image with an embedded manifest', async () => {
      ...
      expect(ingredient.manifest_data?.identifier).toMatch(MANIFEST_DATA_IDENTIFIER);
